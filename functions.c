...
lval* builtin_head(lenv* e, lval*a) {
  ...
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR, "Function 'head' passed incorrect type for argument 0. ", "Got %i, expected %i.", ltype_name(a->cell[0]->type), ltype_name(LVAL_QEXPR));
...
lval* builtin_op(lenv* e, lval* a, char* op) {
  ...
  // If no arguments and sub, perform unary operation
...
lval* builtin_var(lenv* e, lval* a, char* func) {
  ...
  LASSERT(a, syms->count == a->count-1, "Function '%s' passed too many arguments for symbols. Got %s, expected %s.", func, syms->count, a->count-1);
...
lval* lval_call(lenv* e, lval* f, lval* a) {
  ...
  if (f->formals->count > 2 && strcmp(f->formals->cell[0]->sym, "&") == 0) {
